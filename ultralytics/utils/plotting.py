    ...
    for i, f in enumerate(files):
        try:
            ...
            if i == 0:
                ...
                # Foundation observability can add an odd number of loss
                # columns. Allocate a ceiling half-grid so the final series
                # never indexes past the flattened axes array.
                n_axes = max(1, (len(columns) + 1) // 2)
                fig, ax = plt.subplots(2, n_axes, figsize=(len(columns) + 2, 6), tight_layout=True)
                ax = ax.ravel()
