...
Write-Host "Starting P1 worker probe: YOLO26n + VisDrone small-GT Dynamic TopK, 10 epochs" -ForegroundColor Cyan
Write-Host "Throughput settings: batch=16, workers=2" -ForegroundColor Cyan
...
conda run --no-capture-output -n yolo_master yolo detect train `
    ...
    epochs=10 imgsz=640 batch=16 device=0 workers=2 `
    ...
    project="D:/coding/YOLO-Master/A2/runs" name="p1_dynamic_topk_a08_y26n_vd640_s42_10e_fast_b16_w2" exist_ok=False
