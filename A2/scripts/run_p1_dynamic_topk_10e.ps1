...
Write-Host "Starting P1: YOLO26n + VisDrone small-GT Dynamic TopK, 10 epochs" -ForegroundColor Cyan
Write-Host "Small GTs: K=ceil(0.8*x), x=candidate points in the baseline-expanded box" -ForegroundColor Cyan
Write-Host "Medium/large GTs: fixed baseline K=10" -ForegroundColor Cyan
...
conda run --no-capture-output -n yolo_master yolo detect train `
    ...
    epochs=10 imgsz=640 batch=2 device=0 workers=0 `
    ...
    project="D:/coding/YOLO-Master/A2/runs" name="p1_dynamic_topk_a08_y26n_vd640_s42_10e" exist_ok=False
