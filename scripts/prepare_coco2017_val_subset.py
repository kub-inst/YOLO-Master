"""Create a reproducible image-list subset from local COCO2017 val2017."""
...
def parse_args() -> argparse.Namespace:
    """Parse validation subset generation options."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--dataset-root", type=Path, default=DEFAULT_DATASET_ROOT)
    parser.add_argument("--size", type=int, default=100, help="Number of val2017 images to sample.")
...
def main() -> None:
    """Sample COCO validation images and write a matching dataset YAML."""
    args = parse_args()
    image_dir = args.dataset_root / "images" / "val2017"
    source_yaml = REPO_ROOT / "scripts" / "coco2017.yaml"
    if args.size <= 0:
        raise ValueError("--size must be positive")
    if not image_dir.is_dir():
        raise FileNotFoundError(f"COCO validation image directory not found: {image_dir}")
    ...
    stem = f"coco2017_val{args.size}_seed{args.seed}"
    ...
    data["val"] = str(list_path)
    data["subset"] = {"split": "val2017", "size": args.size, "seed": args.seed}
